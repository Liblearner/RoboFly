 ...
 * 备  注：无
 *******************************************************************************/
void MPU6050_AccRead(int16_t *AccX, int16_t *AccY, int16_t *AccZ)
{
	uint8_t buf[6];
 ...
 * 备  注：无
 *******************************************************************************/
void MPU6050_GyroRead(int16_t *GYROX, int16_t *GYROY, int16_t *GYROZ)
{
	uint8_t buf[6];
...
void MPU6050_Init(void)
{
	...
	MPU6050_WriteByte(MPU6050_RA_CONFIG, MPU6050_DLPF_BW_98); // 设置陀螺的输出为1kHZ,DLPF=20Hz
...
{
	static int is_gyro_calib = 0;
	static int is_acc_calib = 0;
	...
	if (!is_acc_calib && is_gyro_calib) // 加速度计进行零偏校准
	{
