...
u8  TX_ADDRESS[TX_ADR_WIDTH]= {0x34,0x43,0x10,0x10,0xF1};	//此地址用来识别接收端哪个RX通道可以接收发送出去的数据包
u8  RX_ADDRESS[RX_ADR_WIDTH]= {0x34,0x43,0x10,0x10,0xF1};	//此地址用来配置本机SI24R1的RX0通道的地址，同时为了能正常收到应答信号，此地址一般都和上面的地址配置相同
